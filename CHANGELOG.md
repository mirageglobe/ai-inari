...
### fixed
- doctor: warns about config settings that look configured but do nothing. an existing config file is never backfilled, so a field added after it was written stays empty, and a key that no longer exists is dropped by the decoder in silence. a real config still carrying the pre-consolidation `models.worker`/`models.sensor` had both ignored and `models.runner` left empty, so `doctor --models` verified only the thinker and still reported green. doctor now lists unrecognised keys and flags an unset runner tier. it warns rather than fails, so the exit code still means "inari can run", and it does not backfill the missing values, which would change which models doctor loads without being asked.
- audit: a tool call the user refuses is now recorded as a `tool.denied` entry (session, tool, args). previously a rejected call was written nowhere except the verbose stderr log, so a refused `curl` left no trace on disk, despite the security model stating that rejections are logged. executed calls keep the `tool.call` method, so counting executions still works and `doctor --models` does not mistake a refused call for a working one.
...
### added
- cli: `inari chat --session <id> --message <text>` runs one turn headlessly and prints the assistant reply (no TUI); `--message -` reads stdin and `--json` wraps it as `{"reply": ...}`. it ensures a background daemon is up, then calls the non-streaming `session.chat` RPC (a plain model round-trip, no tool-call loop) so the path is deterministic and never blocks on an approval prompt - a scriptable entry point for automation and tests. targets an existing session with `--session`, or creates a fresh one with `--new` (below).
- cli: `inari chat --new [--name N] [--model M] [--cwd P] --message <text>` creates a fresh persisted session and runs one headless turn against it, so a headless call no longer needs a pre-existing session; `--new` and `--session` are mutually exclusive, `--model` defaults to the daemon's configured model, and the new session id is echoed to stderr (stdout stays the reply).
