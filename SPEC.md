...
### Near-term
- [ ] `[inari]` `[hard]` **architecture review + refactor** - a whole-app pass for performance and design, optimising for simplicity, extensibility, and maintainability with no behaviour regressions. scope: audit hot paths (the `handleStream` tool-call loop, viewport hardwrap/render on every token, per-turn history copies); the RPC surface and the growing `NewServer` positional-param list (candidate: a config/options struct instead of ~10 positional args); config layering and the `provider` abstraction seam (second-backend readiness); package boundaries and the remaining oversized files. deliver as a written findings doc first, then land incremental refactors as small, separately-reviewable PRs rather than one big-bang change. **audit complete (2026-07-16): [docs/architecture-audit.md](docs/architecture-audit.md) is the dated decision record. refactors landed across PRs #76-#83 (2026-07-16 to 2026-07-18); the second-provider seam is deferred and streaming-render profiling is spun out as the near-term item below.**

- [ ] `[inarid/inarit]` `[medium]` **streaming hot-path profiling (P2/P4/P5)** - measure the token-stream path under a real model before optimising it (measure-first, per the audit decision record). candidates: P2 per-token full-scrollback re-wrap (`chat_stream.go` to `chat_render.go`), P4 `streamBuf += token` quadratic growth, P5 `/api/ps` blocking round-trip per turn. add a benchmark/pprof entry point, run it against a live ollama session, then land the streaming-render cache only if the profile justifies it. see [docs/architecture-audit.md](docs/architecture-audit.md).

### Ideas
- [ ] `[inari]` `[medium]` **headless mode** - run inari without the bubbletea TUI: inarid already runs standalone, so add a non-interactive way to drive a session (e.g. `inari chat --session <id> --message <text>`, or a JSON/stdio mode) so inari can be scripted, piped into other tools, or embedded and tested without the UI. useful for automation, CI, and exercising the daemon in isolation.
