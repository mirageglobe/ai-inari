...
- [ ] `[inarid]` `[hard]` multi-model routing — sensor tier classifies intent, dispatches to worker or thinker
...
- [ ] `[inarid]` `[hard]` **task difficulty/effort classification** — investigate how to define and score task difficulty, complexity, and effort (e.g. token count, tool-call depth, reasoning hops) so inarid can automatically select the appropriate model tier (sensor → worker → thinker) rather than relying on manual session config
...
### Done
- [x] `[inari]` `[medium]` **homebrew release for the inari binary** - `inari` now releases to the `mirageglobe/homebrew-tap` tap via goreleaser, installable with `brew install mirageglobe/tap/inari`. added a `brews` formula block to `.goreleaser.yaml` targeting the tap repo (formula, not cask, to match the tap convention `Formula/scout.rb` and support linux brew; goreleaser 2.17 flags `brews` deprecated but still generates the formula on `release`), and a tag-triggered `.github/workflows/release.yml` that runs goreleaser `release --clean` on a `v*` push (mirrors the scout repo scaffolding). **also fixed a latent release-blocking bug:** `builds.main` was `./cmd/inari/main.go`, a single file, which stopped compiling once the arch-review split `main.go` into sibling files (chat/daemon/doctor/process/paths/tui) - goreleaser built only `main.go` and failed with `# command-line-arguments`; changed to the package path `./cmd/inari`. verified end-to-end with `goreleaser release --snapshot --clean`: builds darwin amd64/arm64 + linux amd64, archives, checksums, and renders `dist/homebrew/Formula/inari.rb` (per-arch url + sha256, correct BUSL-1.1 license), which overwrites the stale hand-written formula (v0.2.0, Apache-2.0) on release. **remaining to publish (operator steps, not code):** add a `HOMEBREW_TAP_GITHUB_TOKEN` repo secret (PAT/deploy key with write to `mirageglobe/homebrew-tap`; the default `GITHUB_TOKEN` cannot push cross-repo), then push a `v*` tag to trigger the release.
