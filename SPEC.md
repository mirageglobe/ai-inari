...
- [ ] `[inarid]` `[easy]` **direct tool tests against a checked-in fixture** - four of the seven declared builtins have no test that ever calls them. `execTool` is exercised directly only for `find_files`, `read_lines` and `execute_shell_command`, which leaves `read_file`, `list_dir`, `grep_file` and `stat_file` with no behavioural coverage: `TestBuiltinToolNamesMatchDeclarations` and `TestSafeToolsAreDeclared` assert only that a name is declared and tiered, never that a call returns the right bytes. there is also no `testdata/` anywhere in the repo; 12 ipc test files each hand-roll their own `t.TempDir()` layout, so no two tests read the same tree and changing the fixture shape means editing twelve files. scope: one checked-in fixture tree (nested directories, a file with a known line count, a file outside the sandbox to attempt escaping to, and a path that does not exist) plus a helper that points a test at it, then one direct `execTool` case per builtin covering the happy path, the cwd-escape rejection (`../..`), and the missing-path error. **deliberately distinct from widening the probe suite above**: probe measures whether a *model* selects the right tool across real streamed turns and needs ollama running, so it cannot gate CI; this measures whether the tool itself returns correct output with no model and no daemon in the loop, so it runs anywhere `go test` does.
