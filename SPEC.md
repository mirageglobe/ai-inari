...
- [ ] `[inari]` `[medium]` **split remaining oversized files**: the four monoliths and the five ipc/session/ollama daemon files are now split (see Done). files still over the ~150-line guideline, for follow-up, all in `tui/views`: `selector.go` (312), `describe.go` (287), `agents_cmds.go` (240); plus minor residuals from earlier splits (`chat.go` 204, `agents_view.go` 176, `chat_render.go` 171, `chat_keys.go` 161, `agents.go` 152, `internal/ipc/tools_exec.go` 154). split by responsibility.
