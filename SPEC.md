...
- [ ] `[inari]` `[medium]` **`!` bash passthrough** - a `!`-prefixed line in the TUI input runs the remainder as a shell command directly, bypassing the model, and streams stdout/stderr into the chat transcript. the `!` is a UX shortcut, not a bypass of the security model: the cwd lock, 30s timeout, and 64KB output cap (§8.3) still apply, and an unlisted binary still prompts for approval. **open decision:** `execute_shell_command` is deliberately `exec.Command(binary, args...)` with no `sh -c`, so it cannot do pipes/redirects/globs; a literal "real bash" `!` must either (a) reuse that safe word-split path and stay allowlist-gated (no pipes), or (b) introduce a real `sh -c` shell, which reopens the injection surface §8.3 closed and needs its own gate. resolve this before implementing; do not silently widen the allowlist.
