...
### Near-term
- [ ] `[inarid]` `[easy]` **audit denied tool calls** - §8.2 layer C states "rejection is logged" and §5.1 describes the audit logger as an "append-only record of all tool calls", but neither holds: `handleStream` writes its `tool.call` entry only after `execTool` returns, and a user-denied call takes the `continue` branch above that, leaving no record anywhere except the verbose stderr log. the audit trail is exactly where you would look to find what a model *tried* to do, so a rejected `curl` or `rm` currently leaves no trace on disk. scope: emit a `tool.denied` entry (session, tool, args) on the denial path and align the §8.2/§5.1 wording with what the code does. found while building `inari probe`, which has to capture denials client-side off the approval channel precisely because they never reach the log; that workaround can be dropped once this lands.
