...
### Near-term
- [ ] `[inarid]` `[medium]` **local project-scoped configuration** - read project-scoped settings (e.g. custom prompts, file exclusions) from a local `.inari/config.json` in the session's working directory, overriding global settings where fields overlap. the global layer shipped (see Done: global context configuration); this is the remaining per-project override layer, deferred pending a decision on merge precedence.
