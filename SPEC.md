...
- [ ] `[inarid/inari]` `[medium]` **split remaining oversized files**: the four originally-flagged monoliths (`chat.go`, `tui/model.go`, `agents.go`, `cmd/inari/main.go`) are now split (see Done). files still over the ~150-line guideline, for follow-up: `internal/ipc/dispatch.go` (341), `tui/views/selector.go` (312), `tui/views/describe.go` (287), `internal/ipc/tools.go` (276), `internal/session/session.go` (271), `tui/views/agents_cmds.go` (240), `internal/ollama/client.go` (227), `internal/ipc/server.go` (226); plus minor residuals in the just-split views package (`chat.go` 204, `agents_view.go` 176, `chat_render.go` 171, `chat_keys.go` 161, `agents.go` 152). split by responsibility.
...
- [ ] `[inarid]` `[medium]` **pre-send message intercept** - validate a chat message locally in inarid before it is sent to the model: reject or flag known-unsafe input (e.g. destructive shell command text, secrets-looking strings) and short-circuit obviously-invalid or empty-effort input with a local response instead of a full Ollama round-trip. this is a latency/cost optimisation, not a replacement for the existing tool-call safety tiers (§8.2/§8.3) which still gate actual tool execution after the model responds.
