...
- [x] `[inarit/inarid]` connects and performs handshake.
- [x] `[inarit/inarid]` basic ping/pong JSON-RPC round-trip.
...
- [x] `[inarit/inarid]` token stream forwarded to inarit chat view.
...
#### M5 — Chat View
- [x] `[inarit]` interactive `i` view wires to Head Inari (Thinker tier).
- [x] `[inarid]` message history scoped to session.
- [x] `[inarit/inarid]` detach/reattach preserves session state.
...
- [ ] `[inarit/inarid]` `[medium]` **model-load vs thinking indicator** - the first message of a fresh session is slow because Ollama cold-loads the model into memory before generating; the current spinner reads "thinking" the whole time and hides this. inarid should detect the load phase (model not yet resident via `ListRunning`/`/api/ps` at send time) and signal a distinct state to inari; the chat spinner then shows `loading <model>...` during the load, switching to `thinking...` once the first token streams. keep it state-accurate: driven by the actual phase, not a fixed timeout.
...
- [ ] `[inarit/inarid]` `[medium]` **delete model from disk** - add a hotkey in the model selector to remove a downloaded model from local storage (Ollama `DELETE /api/delete`), reclaiming disk space. distinct from unload (evict from memory, `UnloadModel`) - this frees disk, not RAM. needs a new `DeleteModel` provider method + `model.delete` RPC, then a list refresh so the row flips back to `[pull]`. destructive and irreversible (re-download required), so it must confirm before deleting per the §8.2 destructive-action policy; never delete the model currently assigned to a session without an explicit extra warning.
...
- [ ] `[inarit/inarid]` session tagging - apply labels to sessions for grouping and quick filtering; plain search is already covered by the near-term agents search/filter item
