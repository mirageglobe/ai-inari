...
- [ ] `[inari]` `[medium]` **split remaining oversized files**: the four monoliths, the five ipc/session/ollama daemon files, and the three flagged `tui/views` files are now split (see Done). minor residuals remain, all just over the ~150-line guideline: `chat.go` 204, `agents_view.go` 176, `chat_render.go` 171, `chat_keys.go` 161, `selector_update.go` 152, `agents.go` 152, `internal/ipc/tools_exec.go` 154. split by responsibility.
- [ ] `[inarid/inari]` `[medium]` **test coverage for untested packages**: `ollama` now covers `Ping`/`ListModels`/`ListRunning`/`Load`/`UnloadModel`/`Chat`/`ChatStream`/`ModelCaps` (81.7% statements) alongside the existing `PullModel` tests; `provider` is pure types/interface with no logic to test. remaining zero-test packages: `mcp`, `version`, `tui`, `cmd/inari`; `tui/views` render seams (chat, agents) are still untested. add unit coverage, prioritising the `tui/views` render seams next.
...
- [ ] `[inarid]` `[medium]` **pre-send message intercept** - validate a chat message locally in inarid before it is sent to the model: reject or flag known-unsafe input (e.g. destructive shell command text, secrets-looking strings) and short-circuit obviously-invalid or empty-effort input with a local response instead of a full Ollama round-trip. this is a latency/cost optimisation, not a replacement for the existing tool-call safety tiers (§8.2/§8.3) which still gate actual tool execution after the model responds. this is the security/validation half of the pre-send intercept layer; the prompt-optimisation half is the item below.
- [ ] `[inari]` `[medium]` **rolling usage hints on idle** - when the chat view has no activity, surface a rotating "how to use inari" hint (e.g. `try /compact to summarise a long chat`, `ctrl+o opens the model selector`) in an unobtrusive line (hint/status line preferred over the chat viewport so history is not disturbed). an idle timer resets on any keypress or stream token; after 60s of no activity the current hint shows, cycling to the next hint every 60s thereafter. hints are drawn from a static pool; stop cycling once the user acts.
- [ ] `[inari]` `[easy]` **more built-in themes** - add ~3 more colour themes alongside the existing default purple, amber, slate, rose (cycled with `[t]`, stored in `config.json`). candidates: emerald, cyan, mono/greyscale. each defines the same palette slots as the existing themes so no render changes are needed.
- [ ] `[inari/inarid]` `[medium]` **model-load vs thinking indicator** - the first message of a fresh session is slow because Ollama cold-loads the model into memory before generating; the current spinner reads "thinking" the whole time and hides this. inarid should detect the load phase (model not yet resident via `ListRunning`/`/api/ps` at send time) and signal a distinct state to inari; the chat spinner then shows `loading <model>...` during the load, switching to `thinking...` once the first token streams. keep it state-accurate: driven by the actual phase, not a fixed timeout.
- [ ] `[inari]` `[medium]` **pre-send prompt optimisation (autocorrect)** - the prompt-optimisation half of the pre-send intercept layer (its security/validation half is **pre-send message intercept** above, which stays daemon-side as the authoritative gate). client-side because it is a UX affordance, not a security control: before the message leaves the TUI, lightly rewrite it to improve model accuracy - fix obvious typos, expand terse fragments, normalise formatting - without altering intent. the user must preview the rewrite in the input box and accept or reject it before send (or toggle the feature off); it never silently distorts what the user asked.
- [ ] `[inari]` `[easy]` **rename TUI references to `inarit`** - the TUI client is referred to inconsistently as "kitsune" and "inari tui" across the docs (kitsune survives only in SPEC.md, README.md, CHANGELOG.md; the code already uses `inari`/`tui`). standardise on a modality-based naming family, single trailing letter matching the `inarid` daemon convention: `inari` = product/umbrella, `inarid` = daemon, `inarit` = terminal ui (this client), with `inarig` (desktop/gui) and `inariw` (web) reserved for future frontends. modality-based rather than role-based ("inariui") so the scheme survives a second frontend. sweep the docs and, critically, resolve the overloaded `[...]` component tags in this roadmap - `[kitsune]` becomes `[inarit]`, and each existing `[inari]` tag is reclassified as either `[inarit]` (TUI work: hints, themes, session search) or left `[inari]` (genuinely product-level). docs/tags only for now; renaming the `cmd/inari` binary or `tui` package is a separate, larger decision, not part of this item.
- [ ] `[inari]` `[easy]` **fold `/model unload` into a hotkey** - drop `/model unload` as a standalone slash command and expose unload as a hotkey inside the model selector modal (e.g. `[u]` on the loaded model), where the model context already lives. removes one entry from the `chatCommandTable` and its dimming guard; keeps the model actions (select, pull, unload) together in one surface instead of split between a modal and a slash command.
- [ ] `[inari/inarid]` `[medium]` **change session cwd** - cwd is currently fixed at session creation; allow switching an existing session to a different working directory. inari offers a path picker/entry; inarid handles a `session.setcwd` RPC that updates the stored cwd, re-injects the filesystem-context system prompt for the new tree (layer 1), re-reads the `AGENTS.md`/`.inari/context.md` project context, and re-points the tool-call sandbox (`sandboxPath` in `internal/ipc/tools.go`) at the new path so shell/file tools stay confined. footer cwd line updates on success. mirrors the `session.rename` RPC pattern in Ideas.
- [ ] `[inari]` `[easy]` **consistent modal width** - popups render at differing widths: the model selector pins `modalInnerW = 64` (`tui/views/selector_render.go`) while the agents modal (`agents_view.go`) sizes off `h.width`, and describe/help overlays differ again. extract a single shared modal-width constant (capped to the 100-char TUI budget) and route every `RenderModal`/overlay through it so all popups share one width. keep the `tea.WindowSizeMsg`-driven clamp so narrow terminals still fit.
- [ ] `[inari]` `[easy]` **status column in model popup** - add a `status` column to the model selector table (`tui/views/selector_render.go`, currently `model` + `est. vram`) showing `downloaded` when the model is present locally, versus the `[pull]` state already used for recommended-but-not-pulled models. the local-vs-recommended data is already computed to mark `[pull]`; this promotes it from an inline name suffix to a proper column. natural extension: also show `loaded` for models resident in memory (`ListRunning` already available). recompute column widths within the shared modal-width budget when adding the column.
- [ ] `[inari/inarid]` `[medium]` **delete model from disk** - add a hotkey in the model selector to remove a downloaded model from local storage (Ollama `DELETE /api/delete`), reclaiming disk space. distinct from unload (evict from memory, `UnloadModel`) - this frees disk, not RAM. needs a new `DeleteModel` provider method + `model.delete` RPC, then a list refresh so the row flips back to `[pull]`. destructive and irreversible (re-download required), so it must confirm before deleting per the §8.2 destructive-action policy; never delete the model currently assigned to a session without an explicit extra warning.
- [ ] `[inari]` `[easy]` **larger agent-name pool** - `sessionAdjectives` in `tui/views/agents_fmt.go` has only 24 adjectives (paired with "agent"), so name generation exhausts to `agent #N` after 24 sessions. either extend the adjective list, or better, pair adjective + a noun pool so N names become N×M combinations for almost no extra code (mirrors the global branch-name word bank). keep `pickAgentName`'s in-use dedup and the numeric fallback intact.
- [ ] `[inari]` `[easy]` **model notes column in popup** - add a `notes` column to the model selector table sourced from each model's `Notes` (already on `CuratedModel` in `tui/views/curated.go`, currently unused in the selector); column header named `notes` to match the field and keep the label short. truncate the cell to a single line with a trailing `...` so long notes never wrap or spill; cut on rune boundaries measured with `lipgloss.Width`, not bytes. coordinate column widths with the `status` and `est. vram` columns so the table (model + status + notes + vram) stays within the shared modal-width budget.
