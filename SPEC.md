...
- [ ] `[inari]` `[medium]` **homebrew release for the inari binary** - publish `inari` to the `mirageglobe/tap` homebrew tap so it installs via `brew install mirageglobe/tap/inari`. reference the scout repo's release scaffolding (`mirageglobe/scout`: `.goreleaser.yaml` + `.github/workflows/release.yml`, goreleaser `release --clean` on a `v*` tag). inari already mirrors that goreleaser (builds darwin/linux amd64/arm64, archives, checksum, github release) but has no release workflow and no tap publish. scope: add a `brews:` block to `.goreleaser.yaml` targeting the tap repo (goreleaser generates + commits the formula on release), add a tag-triggered `release.yml` mirroring scout's, and supply a tap-push token (a PAT/deploy key for `mirageglobe/homebrew-tap`, which exists; the default `GITHUB_TOKEN` cannot push to another repo). note: scout documents the tap install but its own goreleaser has no `brews:` block, so the formula automation is the piece to add, not copy.
