...
### Done
- [x] `[inari]` `[medium]` **homebrew release for the inari binary** - `inari` now releases to the `mirageglobe/homebrew-tap` tap via goreleaser, installable with `brew install mirageglobe/tap/inari`. added a `homebrew_casks` block to `.goreleaser.yaml` (goreleaser 2.17 deprecates `brews`/formulae for pre-built binaries in favour of casks) targeting the tap repo, and a tag-triggered `.github/workflows/release.yml` that runs goreleaser `release --clean` on a `v*` push (mirrors the scout repo scaffolding). **also fixed a latent release-blocking bug:** `builds.main` was `./cmd/inari/main.go`, a single file, which stopped compiling once the arch-review split `main.go` into sibling files (chat/daemon/doctor/process/paths/tui) - goreleaser built only `main.go` and failed with `# command-line-arguments`; changed to the package path `./cmd/inari`. verified end-to-end with `goreleaser check` and `goreleaser release --snapshot --clean`: builds darwin amd64/arm64 + linux amd64, archives, checksums, and renders `dist/homebrew/Casks/inari.rb` (per-arch sha256 + release-asset urls, `binary "inari"`). **remaining to publish (operator steps, not code):** add a `HOMEBREW_TAP_GITHUB_TOKEN` repo secret (PAT/deploy key with write to `mirageglobe/homebrew-tap`; the default `GITHUB_TOKEN` cannot push cross-repo), then push a `v*` tag to trigger the release.
