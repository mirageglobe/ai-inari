...
### Near-term
- [ ] `[inarid]` `[easy]` **warn on stale config keys and empty model tiers** - `config.Load` does no backfill for an existing file, so a config written before a field existed silently gets the zero value, and keys that no longer exist are dropped by the json decoder with no signal. measured on a live config still carrying the pre-consolidation `models.worker`/`models.sensor`: both are silently ignored, `models.runner` is empty as a result, and `doctor --models` therefore verifies only the thinker while still reporting green. scope: have `inari doctor` decode into a map alongside the struct and report unknown keys plus empty tiers. deliberately not a silent backfill - quietly materialising a runner model would change which models doctor loads and runs without the user asking for it.
