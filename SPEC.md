...
### Near-term
- [ ] `[inarid]` `[easy]` **memoize `/api/show` context-length lookup (P3)** - `handleStream` calls `provider.ModelContextLength` (`/api/show`) every turn to derive a static `num_ctx`; measured ~7-29ms/turn against live ollama (spun out of the streaming hot-path profiling, see Done). cache it per model, invalidated on model change/unload, so it is fetched once rather than per turn. low urgency (immaterial vs multi-second inference) but a clean redundant-call removal.
