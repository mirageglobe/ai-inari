...
- [ ] `[inari]` `[medium]` **discover + locally test new candidate models** - a separate flow (not doctor) for evaluating models you do not yet run: given a candidate tag, or a discovery source (the ollama registry / the curated list), pull it and drive it through the same headless tool-calling smoke test, reporting whether it resolves (no 404/pull error), runs, and actually invokes tools locally. reuses the streaming tool-calling verification shipped for `inari doctor --models` (`verifyOne` in `cmd/inari/doctor_models.go`: fixture cwd + `client.ChatStream` + audit-log `tool.call` check). **note:** `inari chat`/`session.chat` is a plain provider round-trip with no tool loop, so verification must drive the stream path, not `chat`. this is the concrete mechanism the curated-model-list rebuild (above) needs for its own "verify each tag actually pulls" step, and doubles as that item's automation investigation, without bloating doctor's preflight scope with market-wide model testing.
