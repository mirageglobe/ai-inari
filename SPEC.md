...
### Near-term
- [ ] `[inarit/inarid]` `[medium]` **rename session** - allow renaming an existing session from the agents view; inari sends a `session.rename` RPC that updates the stored name and propagates it to all open views. mirrors the `session.setcwd`/`session.setcontext` handler pattern already in `dispatch_session.go`.
- [ ] `[inarid]` `[medium]` **auto-compression threshold** - automatically trigger `/compact` when a session's context exceeds a configurable fraction of `num_ctx` (e.g. 80%); inarid monitors context size after each turn and fires the existing summarisation pipeline without user intervention. builds directly on the shipped num_ctx detection and `/compact`.
- [ ] `[inarid]` `[medium]` **model loop / EOF prevention** - some models enter repetitive generation loops (e.g. `for_for_for...`) that exhaust the context window and terminate the stream with an EOF error. mitigations: (1) set `repeat_penalty` (1.3-1.5) and a `num_predict` cap in the ollama request options; (2) add a stream-side n-gram detector in `handleStream` that cancels the stream gracefully when a short sequence repeats 3+ times consecutively.
- [ ] `[inarid]` `[medium]` **MCP tool-call dispatch** - `internal/mcp/host.go` `Call()` is a TODO stub; audit logging exists but actual JSON-RPC dispatch over stdio is not implemented. complete to fulfil M4.
- [ ] `[inarit/inarid]` `[medium]` **session tagging** - apply labels to sessions for grouping and quick filtering; builds on the shipped agents search/filter (plain-text search is already covered).
- [ ] `[inarit]` `[hard]` **chat viewport character selection** - build on line selection to add character-level precision: within the selected rows, re-parse ANSI sequences to locate byte ranges and inject highlight styles mid-sequence, so a drag can start and end partway through a line. builds on the shipped line-selection work (see Done).
- [ ] `[inarid/inarit]` `[medium]` **per-session context window override (num_ctx UI)** - detection, display, and a capped default shipped (see Done). remaining scope: let the user view and adjust `num_ctx` per session from the chat view (a hotkey plus input), and replace the flat `min(max, 8192)` cap with per-tier defaults (higher for thinker, lower for sensor) once a session-to-tier mapping exists. inarid would store the override on the session and prefer it over the computed default.
...
- [ ] `[inarit]` `[medium]` **split remaining oversized files**: `agents_view.go` (206->131, table+accessors extracted to `agents_table.go`) and `selector_update.go` (187->124, key handling extracted to `selector_keys.go`) are now split (see Done). remaining residuals, all just over the ~150-line guideline: `chat.go` 240, `chat_render.go` 182, `chat_keys.go` 165, `agents.go` 158, `internal/ipc/tools_exec.go` 156. the three `chat*` files are deferred until the idle-hints and interrupt PRs land (they touch the same files); `agents.go`/`tools_exec.go` are marginal (6-8 lines over) and low value to split. split by responsibility.
- [ ] `[inarid/inarit]` `[medium]` **test coverage for untested packages**: `ollama` now covers `Ping`/`ListModels`/`ListRunning`/`Load`/`UnloadModel`/`Chat`/`ChatStream`/`ModelCaps` (81.7% statements) alongside the existing `PullModel` tests; `provider` is pure types/interface with no logic to test. remaining zero-test packages: `mcp`, `version`, `tui`, `cmd/inari`; `tui/views` render seams (chat, agents) are still untested. add unit coverage, prioritising the `tui/views` render seams next.
