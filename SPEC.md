...
### Near-term
- [ ] `[inarit/inarid]` `[medium]` **rebuild the curated model list for local LLMs + investigate automation** - the curated recommendation table (`tui/views/curated.go` `CuratedModels`, hand-mirrored in SPEC §6.1) and the config default tiers (`models.thinker/runner`) are hand-picked and unvalidated; some tags are not currently pullable (confirmed: the former `bonsai:4b`/`qwen3-nano` worker/sensor defaults 404 against a live ollama, which is part of why those two tiers were consolidated into one `runner` tier - see §2.2). `runner` currently ships with no default (empty), pending this rebuild. rebuild both with real, currently-available models that run well locally, sized to the 4/8/16/32gb tiers and general/coding roles, and verify each tag actually pulls. **automation (investigate):** the table is dual-maintained (curated.go + SPEC §6.1) so it drifts; explore keeping it current - generate both from one data source, validate tags against ollama (`/api/show` or the registry) via `inari doctor` or a `make` target, and/or surface locally-pulled models the list omits. deliver the rebuilt list first, then a short findings note on the automation options before committing to one.
...
- [ ] `[inari]` `[medium]` **`inari doctor`: verify pulled models actually work** - today doctor only checks that a configured tag is *present* (`modelPresent` name-match in `cmd/inari/doctor.go`). presence is not function: a pulled model can still fail to load, error on chat, or never emit a tool call. drive each *configured* model (thinker, and runner when set) through a real headless turn against a small fixture cwd and confirm it (1) responds without error and (2) invokes a tool when the prompt requires one, checking the audit log (`internal/audit`, `Auditor.Log`) for the tool-call entry rather than parsing the reply text - a model can narrate a tool result without one having actually run (the exact failure the "surface tool output on empty final answer" fix addressed). mechanics: reuse the existing headless `inari chat --new --model <tag> --cwd <fixture-dir> --message <prompt>` path (already scriptable, no TUI). **scope: only models already pulled/configured on this machine** - doctor is a preflight health check, not a model-shopping tool. exit non-zero on a configured model that fails.
- [ ] `[inari]` `[medium]` **discover + locally test new candidate models** - a separate flow (not doctor) for evaluating models you do not yet run: given a candidate tag, or a discovery source (the ollama registry / the curated list), pull it and drive it through the same headless tool-calling smoke test, reporting whether it resolves (no 404/pull error), runs, and actually invokes tools locally. reuses the headless `inari chat --new --model <tag> --cwd <fixture>` path + audit-log check shared with the doctor item above. this is the concrete mechanism the curated-model-list rebuild (above) needs for its own "verify each tag actually pulls" step, and doubles as that item's automation investigation, without bloating doctor's preflight scope with market-wide model testing.
- [ ] `[inarid]` `[easy]` **background the daemon by default; `-f`/`--foreground` to attach** - `inari daemon` currently blocks in the foreground ("ctrl+c to quit") and the existing `--background` flag is a misnomer: it does not detach, it only writes a pid file and skips the ctrl+c line (it is really the internal "you are the forked child" marker set by `cmdStart` in `process.go`, which does the actual fork+return for bare `inari`/`inari start` alongside launching the TUI). so there is no daemon-only background command. scope: make `inari daemon` self-detach by default (reuse the `cmdStart` fork path minus the TUI), add `-f`/`--foreground` to run attached (current behaviour), and rename the internal `--background` child-marker to something unambiguous (e.g. `--child`) so it does not read as the opposite of `--foreground`. side benefit: a foreground daemon writes no pid file today so `inari stop` cannot stop it; backgrounding by default makes every daemon stoppable uniformly. **CLI-contract change:** `make run-daemon` (`go run ./cmd/inari daemon`) and any launchd/systemd unit expect `daemon` to block, so they must move to `daemon -f`; update `run-daemon` in the same change and note the supervisor caveat in help + README/SPEC.
