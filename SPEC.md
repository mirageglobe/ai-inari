...
- **session** - a persistent conversation container: cwd, model assignment, message history, tags. this is what the TUI's `Agents` view currently lists and lets you create/select. a session on its own does nothing autonomous, so calling it an "agent" overstates it; renaming that view to `Sessions` is tracked as a near-term item below.
...
- [ ] `[inari]` `[medium]` **model verification harness (pull + smoke-test tool-calling)** - extend `inari doctor` with a mode that pulls a candidate model tag and drives it through a real headless turn to confirm both (1) the tag resolves (no 404/pull error) and (2) the model actually invokes tools when prompted to, not just returns a plain-text reply. mechanics: reuse the existing headless `inari chat --new --model <tag> --cwd <fixture-dir> --message <prompt>` path (already scriptable, no TUI) against a small fixture directory with known contents, sending a prompt that requires a tool call (e.g. "list the files here"); check the daemon's audit log (`internal/audit`, `Auditor.Log`) for a matching tool-call entry rather than parsing the reply text - a model can narrate a tool result without one having actually run (the exact failure the "surface tool output on empty final answer" fix addressed). exit non-zero on pull failure, no-tool-call, or timeout. this is the concrete mechanism the curated-model-list rebuild (above) needs for its own "verify each tag actually pulls" step, and doubles as that item's automation investigation - build this first, then decide whether it also runs unattended over the whole curated table.
- [ ] `[inarit]` `[medium]` **rename `Agents` view/files to `Sessions`** - a session (cwd, model assignment, history, tags) is not an autonomous agent; it only acts when the user sends a message and waits for a reply (see §2.2 Terminology). the current `Agents` view/naming conflates the two, which will get more confusing once runner agents exist and people are talking about "agents" in two different senses. measured blast radius: 11 files in `tui/views` named `agents_*.go` (`agents.go`, `agents_data.go`, `agents_mutations.go`, `agents_input.go`, `agents_table.go`, `agents_view.go`, `agents_cmds.go`, `agents_msgs.go`, `agents_fmt.go`, plus two `_test.go` files), the `/agent` chat sub-commands, `Agents.Filtering()`, and 34 mentions across SPEC.md plus the README quick-reference. scope: rename files/types/hotkey docs, keep `/agent add`/`/agent chat` etc. working (either renamed to `/session` or kept as an accepted shorthand - decide during the work), no behaviour change. deliberately not folded into the thinker/runner rename above since it is a much larger, purely-cosmetic surface change with no functional payoff on its own.
