...
- [ ] `[inari]` `[easy]` **status column in model popup - `loaded` state** - the `status` column now shows `downloaded` / `[pull]` (see Done). remaining scope is the `loaded` state for models resident in memory; the selector does not currently fetch `ListRunning`, so this needs a running-models fetch wired into the selector's model list load.
