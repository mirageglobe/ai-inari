...
- [ ] `[inarit]` `[medium]` **modal overlays for read-only views** - render `/help`, `/describe`, `/logs`, and the `/tools` panel as pop-up modal overlays drawn over the chat view, rather than inline transcript content or full-screen view swaps. the model selector and `/help` overlay already sit on top of the chat; bring describe, logs, and the tools panel into the same pattern so the chat stays visible underneath and the transcript is never polluted with view output. refer to the existing model-selector and help overlays as the reference implementation.
- [ ] `[inarit]` `[easy]` **uniform modal exit (q/esc -> chat)** - every pop-up modal (help, describe, logs, tools, model selector) must exit on both `q` and `esc` and return to the chat view. audit each overlay's key handler for a single consistent close binding so there is no per-screen surprise; today some views differ on which key dismisses them.
